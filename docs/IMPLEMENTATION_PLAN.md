...
## Task 13. 로그인 실패 제한과 세션 보안 강화

상태: [x]

### 목표

로그인 실패를 무제한으로 시도할 수 없도록 제한하고, 성공/실패 추적과 세션 만료, 비밀번호 기본 정책을 강화한다.
...
- 이메일별 로그인 실패 횟수 제한
- IP별 로그인 실패 횟수 제한
- 성공 로그인 시 같은 이메일/IP 조합의 실패 카운트 초기화
- 세션 절대 만료와 유휴 만료 적용
- 회원가입 비밀번호 정책 강화
- 로그인 차단 감사 로그 기록
...
- `migrations/008_create_login_attempts.sql`
...
- 같은 이메일로 10분 안에 5회 로그인 실패 후 추가 로그인 시도가 차단된다.
- 같은 IP에서 10분 안에 20회 로그인 실패 후 추가 로그인 시도가 차단된다.
- 로그인 성공 시 같은 이메일/IP 조합의 실패 카운트가 초기화된다.
- 세션은 12시간 절대 만료와 2시간 유휴 만료가 적용된다.
- 비밀번호는 10자 이상이어야 하며 이메일 또는 이메일 계정명, 흔한 비밀번호를 사용할 수 없다.
- 로그인 실패, 차단, 성공이 추적 가능하다.

### 작업 결과

- `login_attempts` 테이블 마이그레이션을 추가했다.
- `LoginAttemptService`를 추가해 이메일별 10분 5회, IP별 10분 20회 제한을 구현했다.
- 로그인 실패 시 `login_attempts`와 기존 `audit_logs`에 기록하도록 했다.
- 제한에 걸린 로그인은 `login_blocked` 감사 로그를 남기도록 했다.
- 로그인 성공 시 `login_attempts`에 성공 기록을 남기고 같은 이메일/IP 조합의 실패 기록을 초기화하도록 했다.
- 세션에 12시간 절대 만료와 2시간 유휴 만료를 적용했다.
- 회원가입 비밀번호 최소 길이를 10자로 높이고, 이메일 계정명 포함 및 흔한 비밀번호를 거부하도록 했다.

### 검증 결과

- `migrations/008_create_login_attempts.sql`을 로컬 MySQL에 적용했다.
- `docker compose exec app php -l app/Services/LoginAttemptService.php` 성공.
- `docker compose exec app php -l app/Controllers/AuthController.php` 성공.
- `docker compose exec app php -l app/Services/AuthService.php` 성공.
- `docker compose exec app php -l app/Services/SessionService.php` 성공.
- `docker compose exec app php -l index.php` 성공.
- 로컬 API 로그인 실패를 6회 호출해 1~5회는 `401`, 6회는 `429`와 `로그인 실패 횟수가 많아 10분 후 다시 시도해야 합니다.` 응답을 확인했다.
- Playwright 모바일 폭 390px에서 `/signup`의 비밀번호 `minlength=10`과 가로 스크롤 없음(`scrollWidth=390`)을 확인했다.

### 메모

- 운영에서 더 강한 정책이 필요하면 제한 기준을 환경변수로 분리할 수 있다.
